...
from bert import MAG_BertForSequenceClassification, MAG_BertWithARL, BertWithARL
...
class Seq2SeqModel:
    ...
    def get_learner_optimizer(self):
        ...
        adversary_param_optimizer = list(self.model.get_adversary_named_parameters()) if self.model_type[-3:] == "arl" else []   
    ...
    def test_score_model(self, test_dataloader: DataLoader, use_zero=False):
        if self.args.wandb:
            wandb.init(project=self.args.wandb, settings=wandb.Settings(start_method="fork"), config={**vars(self.args)})
        ...
        pearson = pearsonr(preds, y_test)
        spearman = spearmanr(preds, y_test)
        ...
        if self.args.wandb:
            wandb.log({"mae": mae, "r.stat": pearson.statistic, "r.pvalue": pearson.pvalue, "s.stat": spearman.statistic, "s.pvalue": spearman.pvalue, "p": prec, "r": rec, "f1": f_score})

        return mae, pearson, spearman, prec, rec, f_score
