...
def segment_audio_file(audio_file_path, segments_time_windows, output):
	...
	for (i, begin, end) in segments_time_windows:
		...
		audio_segment.export("{output}/{name}[{no}][{idx}].wav".format(output=output_file_path, name=filename, no=i, idx=idx), format="wav")
		if i == last_i:
			idx += 1
		else:
			idx = 0
...
def segment_video_file(video_file_path, segments_time_windows, output):
	...
	for (i, begin, end) in segments_time_windows:
		...
		if not os.path.exists(output_file_path):
			os.makedirs(output_file_path)
		ffmpeg_extract_subclip(video_file_path, begin, end, targetname="{output}/{name}[{no}][{idx}].mp4".format(output=output_file_path, name=filename, no=i, idx=idx))
		if i == last_i:
			idx += 1
		else:
			idx = 0
