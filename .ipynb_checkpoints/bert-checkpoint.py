...
class MAG_BertWithARL(BertPreTrainedModel):
    def __init__(self, config, multimodal_config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def get_learner_named_parameters(self):
        return self.classifier.get_learner_named_parameters()

    def get_adversary_named_parameters(self):
        return self.classifier.get_adversary_named_parameters()

class BertWithARL(BertPreTrainedModel):
    def __init__(self, config, multimodal_config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(
        self,
        input_ids,
        visual, # not used, but necessary to extend the code in model.py
        acoustic, # not used, but necessary to extend the code in model.py
        attention_mask=None,
        token_type_ids=None,
        position_ids=None,
        head_mask=None,
...
class MAG_RobertaModel(RobertaPreTrainedModel):
    def __init__(self, config, multimodal_config):
        super().__init__(config)
        self.config = config
...
class MAG_RobertawithARL(RobertaPreTrainedModel):
    def __init__(self, config, multimodal_config):
        super().__init__(config)
        self.num_labels = config.num_labels

        self.bert = MAG_RobertaModel(config, multimodal_config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = ARL(config.hidden_size, config.num_labels)
